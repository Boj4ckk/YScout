...
def test_get_youtube_suggestions_list(mock_get):
    ...
    result = CartographyUtils.get_youtube_suggestions_list("test")
    assert result == ["suggestion1", "suggestion2"]

@patch.object(CartographyUtils, "get_youtube_suggestions_list")
@patch("os.getenv")
def test_get_youtube_suggestions_by_keywords(mock_getenv, mock_suggestions):
    ...
    result = CartographyUtils.get_youtube_suggestions_by_keywords()
    assert result == {
        "foo": ["foo_1", "foo_2"],
        "bar": ["bar_1", "bar_2"]
    }
